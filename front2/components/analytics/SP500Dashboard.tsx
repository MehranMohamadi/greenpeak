...
type StatColorKey = "blue" | "purple" | "green" | "indigo" | "orange" | "teal"
...
export default function SP500Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const priceChangePercent =
      previous.close !== 0 ? (priceChange / previous.close) * 100 : 0
    ...
    if (chartData.length >= 10) {
      ...
      const prev5Avg =
        chartData.slice(-10, -5).reduce((sum, d) => sum + d.close, 0) / 5
      momentum = last5Avg - prev5Avg
    }

    const estimatedMarketCap = currentPrice * 500 * 1_000_000
    ...
    () => [
      {
        ...
        change: loading
          ? "..."
          : `${stats.priceChange >= 0 ? "+" : ""}${formatPrice(
              stats.priceChange
      ...
      },
      {
        ...
        change: loading
          ? "..."
          : `Spread: ${formatPrice(stats.dayHigh - stats.dayLow)}`,
      ...
      },
      {
        ...
        change: loading
          ? "..."
          : `${stats.priceChange >= 0 ? "+" : ""}${formatMarketCap(
              Math.abs(stats.priceChange * 500 * 1_000_000)
            )}`,
        changeColor:
          stats.priceChange >= 0
            ? "text-green-600 dark:text-green-400"
      ...
      },
      {
        ...
        value: loading
          ? "Loading..."
          : `${formatPrice(stats.weekLow)} - ${formatPrice(stats.weekHigh)}`,
        change: loading
          ? "..."
          : `Spread: ${formatPrice(stats.weekHigh - stats.weekLow)}`,
        changeColor: "text-gray-600 dark:text-gray-400",
        icon: Calendar,
        color: "orange",
      },
      ...
      >
            ...
            >
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button
                  variant="outline"
                  size="sm"
                  className="flex items-center gap-2 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm border border-white/20 dark:border-gray-700/20"
        ...
        >
          {statCards.map((stat, index) => {
            ...
            const colorClasses = colorClassesMap[stat.color]
            ...
            return (
              ...
              >
                <Card
                  className={`group hover:shadow-2xl transition-all duration-500 border-2 ${colorClasses.border} bg-white dark:bg-gray-900/50 backdrop-blur-sm min-h-[200px] flex flex-col relative overflow-hidden`}
                >
                  <motion.div
                  ...
                  <CardContent className="pt-0 flex-1 flex flex-col justify-center">
                    <div className="space-y-3">
                      ...
                      <div
                        className={`text-sm font-medium ${stat.changeColor} flex items-center gap-1`}
                      >
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription className="mt-1">
                            Interactive candlestick chart with real-time volume data and
                ...
                >
                  <Card className="border-2 border-gray-200 dark:border-gray-700">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Coming soon - Advanced charting with multiple timeframes and
                        indicators
                      </CardDescription>
                    </CardHeader>
                      ...
                      {[
                        ...
                        <Link key={item.href} href={item.href}>
                          ...
                          >
                            <Button
                              variant="outline"
                              className="w-full justify-start bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 transition-all duration-300"
                            >
