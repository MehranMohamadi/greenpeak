...
function Kpi({ label, value, color }) {
  ...
  return <div className="flex items-center justify-between gap-3 rounded-xl border border-slate-700/60 bg-slate-900/60 p-3"><span className="text-xs text-slate-400">{label}</span><span className={`rounded-full border px-3 py-1 text-xs font-semibold ${colors[color]}`}>{value}</span></div>
}
...
export default function MarketIntelligenceSections({ market }) {
  ...
  useEffect(() => {
    ...
    fetch("/analytics-data/news/sources/alpha_vantage?limit=20", { cache: "no-store", signal: controller.signal })
      ...
      .then(feed => {
        ...
        setNews(items); setSelectedNews(items[0] || null)
      }).catch(() => {})
  ...
  const glance = market.glance_summary || {}
  ...
  return <div className="space-y-6 bg-[#0b1120] p-4 text-slate-100 md:p-6" dir="rtl">
    <section className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
      <Card className="overflow-hidden border-cyan-500/20 bg-gradient-to-bl from-[#172554] via-[#111c33] to-[#0f172a]"><CardHeader><CardTitle className="text-2xl">وضعیت فعلی بازار آمریکا 🇺🇸</CardTitle><div className="flex flex-wrap items-center gap-2"><CardDescription className="text-slate-400">Daily Base Analysis · پایش اخبار هر ۱۰ دقیقه · آخرین تحلیل: {formatTime(market.analysis_generated_at)}</CardDescription><Dialog><DialogTrigger asChild><button type="button" aria-label="منطق به‌روزرسانی تحلیل" className="rounded-full text-cyan-400 hover:text-cyan-300"><CircleHelp className="h-4 w-4" /></button></DialogTrigger><DialogContent className="border-slate-700/70 bg-[#111c33] text-slate-100" dir="rtl"><DialogHeader className="text-right"><DialogTitle>منطق به‌روزرسانی</DialogTitle><DialogDescription className="leading-7 text-slate-300">تحلیل پایه هر روز تولید می‌شود و خوراک خبرهای پراهمیت هر ۱۰ دقیقه پایش و تازه‌سازی می‌شود. زمان نمایش‌داده‌شده، زمان واقعی آخرین تحلیل ذخیره‌شده است.</DialogDescription></DialogHeader></DialogContent></Dialog></div></CardHeader><CardContent><p className="text-base leading-9 text-slate-200">{market.market_story_fa}</p></CardContent></Card>
      <div className="space-y-2"><Kpi label="شرایط کلی بازار" value={statusLabels[summary.market_condition] || "نامشخص"} color="violet" /><Kpi label="ریسک بازار" value={statusLabels[summary.risk_level] || "نامشخص"} color="amber" /><Kpi label="Sentiment" value={statusLabels[summary.sentiment] || "نامشخص"} color="slate" /><Kpi label="شدت تغییر" value={statusLabels[summary.change_intensity] || "نامشخص"} color="cyan" /><Kpi label="اعتماد به تحلیل" value={statusLabels[summary.confidence_level] || "نامشخص"} color={summary.confidence_level === "high" ? "green" : "violet"} /></div>
    </section>

    <section className="grid gap-4 lg:grid-cols-2">
      <Card className="border-slate-700/60 bg-[#111c33]/90"><CardHeader><div className="flex items-center justify-between"><CardTitle className="flex items-center gap-2"><Newspaper className="h-5 w-5 text-cyan-400" />اخبار پرتأثیر</CardTitle><Link href="/analytics/events" className="text-xs text-cyan-400">مشاهده همه</Link></div></CardHeader><CardContent className="space-y-2">{news.length ? news.map(item => <button key={item.item_id} onClick={() => setSelectedNews(item)} className={`w-full rounded-lg border p-3 text-right transition ${selectedNews?.item_id === item.item_id ? "border-cyan-400/60 bg-cyan-500/10" : "border-slate-700/60 bg-slate-950/25 hover:border-cyan-500/30"}`}><div className="flex items-center justify-between gap-2"><Badge variant="outline">{item.importance === "high" ? "اثر بالا" : "بااهمیت"}</Badge><time className="text-[11px] text-slate-500">{formatTime(item.published_at)}</time></div><p className="mt-2 line-clamp-2 text-sm font-semibold leading-6">{item.title}</p></button>) : <p className="text-sm text-slate-400">خبر رتبه‌بندی‌شده‌ای در دسترس نیست.</p>}</CardContent></Card>
      <Card className="border-cyan-500/20 bg-[#111c33]/90"><CardHeader><CardTitle>تحلیل خبر منتخب</CardTitle><CardDescription className="text-slate-400">خلاصهٔ منبع؛ تحلیل عمیق فقط در صورت وجود شواهد معتبر ساخته می‌شود.</CardDescription></CardHeader><CardContent>{selectedNews ? <div className="space-y-4"><h3 className="font-semibold leading-7">{selectedNews.title}</h3><p className="text-sm leading-7 text-slate-300">{selectedNews.summary || "این منبع خلاصه‌ای ارائه نکرده است."}</p><div className="flex flex-wrap gap-2"><Badge variant="outline">Relevance: {typeof selectedNews.source_score === "number" ? selectedNews.source_score.toFixed(2) : "—"}</Badge>{selectedNews.alpha_sentiment_label && <Badge variant="outline">{selectedNews.alpha_sentiment_label}</Badge>}</div></div> : <p className="text-sm text-slate-400">یک خبر را انتخاب کنید.</p>}</CardContent></Card>
    </section>

    <section><Card className="border-slate-700/60 bg-[#111c33]/90"><CardHeader><CardTitle>داده‌های مهم منتشرشده</CardTitle><CardDescription className="text-slate-400">فقط اعدادی نمایش داده می‌شوند که در شواهد تحلیل، مقدار واقعی و مقدار مقایسه‌ای داشته باشند.</CardDescription></CardHeader><CardContent>{releasedData.length ? <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="text-slate-400"><tr className="border-b border-slate-700"><th className="p-3 text-right">شاخص</th><th className="p-3">Previous</th><th className="p-3">Forecast</th><th className="p-3">Actual</th><th className="p-3">تحلیل</th></tr></thead><tbody>{releasedData.map(item => <tr key={item.metric} className="border-b border-slate-800"><td className="p-3 text-right"><span className="font-medium">{item.title_fa}</span><span className="mt-1 block text-xs text-slate-500" dir="ltr">{item.metric}</span></td><td className="p-3 text-center tabular-nums" dir="ltr">{item.previous || "—"}</td><td className="p-3 text-center tabular-nums" dir="ltr">{item.forecast || "—"}</td><td className="p-3 text-center font-semibold text-cyan-300 tabular-nums" dir="ltr">{item.current}</td><td className="p-3 text-center"><HelpDialog title={item.title_fa} item={item}><button type="button" aria-label={`تحلیل ${item.title_fa}`} className="text-cyan-400 hover:text-cyan-300"><CircleHelp className="h-4 w-4" /></button></HelpDialog></td></tr>)}</tbody></table></div> : <p className="rounded-lg border border-slate-700/60 bg-slate-950/25 p-4 text-sm text-slate-400">دادهٔ منتشرشدهٔ واجد شرایط در آخرین تحلیل ثبت نشده است.</p>}</CardContent></Card></section>

    <section><Card className="border-slate-700/60 bg-[#111c33]/90"><CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-cyan-400" />رویدادهای مهم پیش‌رو</CardTitle><CardDescription className="text-slate-400">فهرست زیر از خروجی آخرین تحلیل ساخته شده است؛ تاریخ و اهمیت ساختگی نمایش داده نمی‌شود.</CardDescription></CardHeader><CardContent className="space-y-2">{(market.watch_next_fa || []).slice(0, 6).map((item, index) => <div key={`${item}-${index}`} className="flex items-center gap-3 rounded-xl border border-slate-700/60 bg-slate-950/25 p-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-500/15 text-sm font-bold text-cyan-300">{index + 1}</span><p className="flex-1 text-sm">{item}</p></div>)}<Link href="/analytics/events" className="mt-3 inline-flex items-center gap-1 text-sm text-cyan-400">مشاهدهٔ تقویم زنده و تاریخ دقیق <ChevronLeft className="h-4 w-4" /></Link></CardContent></Card></section>

    <section className="grid gap-4 lg:grid-cols-2"><Drivers title="عوامل حمایتی" items={positive} tone="positive" /><Drivers title="عوامل فشاری" items={negative} tone="negative" /></section>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]"><Card className="border-cyan-500/20 bg-gradient-to-br from-[#111c33] to-[#0f172a]"><CardHeader><CardTitle>تحلیل جامع و هم‌افزایی داده‌ها</CardTitle><CardDescription className="text-slate-400">روایت یکپارچهٔ ارتباط سیاست پولی، رشد، تورم، سودآوری و ریسک بازار</CardDescription></CardHeader><CardContent><p className="whitespace-pre-line leading-9 text-slate-200">{market.systemic_synthesis_fa || market.narrative_fa}</p></CardContent></Card><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1"><Kpi label="عوامل حمایتی" value={glance.supportive_fa?.[0] || positive[0]?.title_fa || "ثبت نشده"} color="green" /><Kpi label="عوامل فشاری" value={glance.pressuring_fa?.[0] || negative[0]?.title_fa || "ثبت نشده"} color="amber" /><Kpi label="عدم‌قطعیت اصلی" value={glance.uncertainty_fa?.[0] || "ثبت نشده"} color="violet" /><Kpi label="تغییردهنده وضعیت" value={glance.regime_shifters_fa?.[0] || market.watch_next_fa?.[0] || "ثبت نشده"} color="cyan" /></div></section>
  </div>
}
