...
import { BookOpen, ChevronDown, Dices, Drama, Flower2, HelpCircle, Landmark, Menu, Mic2, X } from "lucide-react";
              ...
              {funNavigation.map((item) => {
                ...
                if (item.type === "category") {
                  ...
                  return <div key={item.id}>
                    <button type="button" aria-label={item.label} aria-expanded={isMashhadOpen} aria-controls={submenuId} title={item.label} onClick={toggleMashhad} onFocus={() => preview(item.label, trail)} onBlur={clearPreview} className={`relative flex h-9 w-full min-w-0 items-center justify-start gap-3 rounded-lg px-3 text-sm transition-colors ${itemFocus} ${neutralItem} ${isMashhadOpen ? "font-medium" : ""}`}>
                      <span aria-hidden="true" className="flex h-5 w-5 shrink-0 items-center justify-center"><Icon className="h-5 w-5" /></span>
                      {showLabels && <span className="flex-1 truncate text-right">{item.label}</span>}
                    ...
                    <AnimatePresence initial={false}>
                      {isMashhadOpen && <motion.div id={submenuId} initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reduceMotion ? 0 : 0.18 }} className="overflow-hidden">
                        <div className="mb-1 mt-0.5 pr-4">
                          {item.children.map((child, index) => {
                            ...
                            return <div key={child.id} className="relative">
                              ...
                              <button type="button" aria-label={child.label} aria-pressed={selected} title={child.label} onClick={() => selectMashhad(child.label, childTrail)} onFocus={() => preview(child.label, childTrail)} onBlur={clearPreview} className={`flex h-9 w-full min-w-0 items-center justify-start gap-3 rounded-lg px-2 text-right text-sm transition-colors ${itemFocus} ${neutralItem} ${selected ? "bg-gray-100 font-medium dark:bg-[#1F1F23]" : ""}`}><span aria-hidden="true" className="flex h-5 w-5 shrink-0 translate-x-1.5 items-center justify-center"><ChildIcon className="h-5 w-5" /></span>{showLabels && <span className="truncate">{child.label}</span>}</button>
                            </div>;
                          })}
                        </div>
                      </motion.div>}
