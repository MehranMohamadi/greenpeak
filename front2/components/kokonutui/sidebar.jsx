...
export default function Sidebar() {
  ...
  return <>
    <button ref={trigger} type="button" aria-label="Open financial navigation" aria-expanded={mobile} aria-controls="financial-navigation" onClick={() => setMobile((value) => !value)} className={`fixed left-4 top-2 z-40 rounded-lg bg-card p-1 lg:hidden ${focusStyle}`}><Menu className="h-5 w-5" /></button>
    <nav id="financial-navigation" aria-label="Financial navigation" dir="ltr"
      ...
      className={`fixed inset-y-0 left-0 z-40 max-w-[calc(100vw-3rem)] border-r border-border bg-card shadow-xl shadow-black/5 transition-[width,transform,visibility] duration-200 motion-reduce:transition-none dark:shadow-black/20 lg:visible lg:translate-x-0 ${mobile ? "visible translate-x-0" : "invisible -translate-x-full"} ${expanded ? "w-64" : "w-16"}`}>
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex h-11 shrink-0 items-center justify-between border-b border-border px-3">
          <Link href="/" aria-label="GreenPeak" title="GreenPeak" className={`flex min-w-0 items-center gap-3 rounded font-medium text-foreground ${focusStyle}`}>
            <svg aria-hidden="true" viewBox="0 0 818 512" fill="none" className="h-8 w-8 shrink-0 text-primary">
          ...
          <button type="button" title={pinned ? "Unpin navigation" : "Pin navigation"} aria-label={pinned ? "Unpin navigation" : "Pin navigation"} aria-pressed={pinned} onClick={() => setPinned((value) => { try { localStorage.setItem("greenpeak.nav.pinned", String(!value)); } catch {} return !value; })} className={`hidden rounded-lg p-2 text-muted-foreground hover:bg-accent hover:text-foreground ${expanded ? "lg:block" : ""} ${focusStyle}`}>{pinned ? <PinOff className="h-4 w-4" /> : <Pin className="h-4 w-4" />}</button>
