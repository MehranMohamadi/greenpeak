...
Keep `current_state_fa`, `what_changed_fa`, and `interpretation_fa` to one short paragraph each. Keep `narrative_fa` to a compact overview and do not repeat the lists. Return the requested indicator JSON contract, including evidence references and explicit data coverage. Do not generate a score or confidence percentage.
